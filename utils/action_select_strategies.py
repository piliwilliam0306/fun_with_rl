import numpy as np
import torch
from epsilon_decay_strategies import ExponentialEpsilonDecay
...
class EpsilonGreedyStrategy():
    def __init__(self, 
                 ...
                 decay_rate,
                 device=torch.device("cuda" if torch.cuda.is_available() else "cpu")):
        self.epsilon_decay = ExponentialEpsilonDecay(max_epsilon, min_epsilon, decay_rate)
        self.device = device
    ...
    def select_action(self, model, state, current_step):
        ...
        epsilon = self.epsilon_decay.get_epsilon(current_step)
